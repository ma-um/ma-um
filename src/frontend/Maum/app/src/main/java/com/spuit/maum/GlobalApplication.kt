...
class GlobalApplication : Application() {
    ...
    fun createRetrofit() {
            ...
            if (checkIsLogin()) {
                ...
                getUserToken()?.let { token -> // token : getUserToken()의 리턴 값
                    val request = original.newBuilder() // 나가려던 통신을 개조한다.
                        .header("Authorization", "token " + 6666) // 헤더를 달아준다.
        ...
        val userRetrofit = Retrofit.Builder()
            .baseUrl("http://192.168.219.100:8091/api/v1/user/")
//            .baseUrl("http://10.0.2.2:8080/music/")
        ...
        val diaryRetrofit = Retrofit.Builder()
            .baseUrl("http://192.168.219.100:8092/api/v1/diary/")
            .addConverterFactory(GsonConverterFactory.create())
            .client(client) // 위에서 만든 클라이언트 넣어줌
            .build()

        val musicRetrofit = Retrofit.Builder()
            .baseUrl("http://192.168.219.100:8093/api/v1/music/")
            .addConverterFactory(GsonConverterFactory.create())
            .client(client) // 위에서 만든 클라이언트 넣어줌
            .build()
    ...
    // SP에 토큰값이 없으면 로그인이 안된거고 있으면 로그인이 되었다고 판단
